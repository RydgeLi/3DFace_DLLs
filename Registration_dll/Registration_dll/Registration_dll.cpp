// Registration_dll.cpp : Defines the exported functions for the DLL application.
//
#include "stdafx.h"
...
void readPointFiles(const char *fielname, vector<Point3D> &P); //too slow!!!
...
void perform(string canonical)
{
	...
	string cannonical_path = "D:\\BS\\eurecom1\\seg\\depth_" + canonical + "_LightOn.txt";
	string segPath = "D:\\BS\\eurecom1\\seg";
	string alignedPath = "D:\\BS\\eurecom1\\aligned\\" + canonical;
	string resultPath = "";
	vector<string> depthFiles;
	getFiles(segPath, depthFiles);

	for (int i = 0; i < depthFiles.size(); i++)
	{
		resultPath = alignedPath + "\\" + depthFiles[i].substr(25, 7) + " - " + canonical + ".txt";
		registration(depthFiles[i].c_str(), cannonical_path.c_str(), resultPath.c_str());
	}
}
...
extern "C" __declspec(dllexport) void perform_shu()
{
	...
	registration_shu(probe_face.c_str(), cannonical_path.c_str(), alignedPath.c_str());
}
